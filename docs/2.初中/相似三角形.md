...
> **课程边界：** 相似三角形的判定、性质以及由平行线形成的基本 A 字型、8 字型属于苏科版课内主干。射影定理、倒数型、旋转手拉手、半角、对角互补、十字架、燕尾与飞鱼等命名模型，以及权数法、斜率法，均作为培优拓展；使用时必须回到相似、全等、勾股或坐标计算给出证明，不能把模型名称当作定理条件。
...
1. **两角分别相等（AA）**
$$\angle A=\angle D,\quad \angle B=\angle E \Longrightarrow \triangle ABC\sim\triangle DEF.$$
2. **两边成比例且夹角相等（SAS）**
$$\frac{AB}{DE}=\frac{AC}{DF},\quad \angle A=\angle D \Longrightarrow \triangle ABC\sim\triangle DEF.$$
3. **三边对应成比例（SSS）**
$$\frac{AB}{DE}=\frac{BC}{EF}=\frac{AC}{DF} \Longrightarrow \triangle ABC\sim\triangle DEF.$$
...
![A 字型、反 A 字型、8 字型与反 8 字型](../img/similarity-a8-models.svg)
...
$$\frac{AD}{AB}=\frac{AE}{AC}=\frac{DE}{BC}, \qquad \frac{AD}{DB}=\frac{AE}{EC}.$$
...
“长度倒数相加”不是由“三条平行线”单独推出的普遍结论。只在具体图形满足以下点序和两组相似比例时使用：$F$ 在线段 $BC$ 上，因而 $BF+FC=BC$，并且两组相似分别给出

$$\frac{BF}{BC}=\frac{EF}{AB},\qquad \frac{FC}{BC}=\frac{EF}{CD}.$$
...
$$1=\frac{BF+FC}{BC} =EF\left(\frac1{AB}+\frac1{CD}\right),$$

因此

$$\frac{1}{EF}=\frac{1}{AB}+\frac{1}{CD}.$$

### 2.4 母子型相似与射影定理【射影定理为培优拓展】
...
$$\triangle ACD\sim\triangle ABC,\qquad \triangle BCD\sim\triangle BAC,\qquad \triangle ACD\sim\triangle CBD.$$
...
$$\frac{AC}{AB}=\frac{AD}{AC} \Longrightarrow AC^2=AD\cdot AB.$$
- 同理，$\triangle BCD\sim\triangle BAC$，所以
$$\frac{BC}{AB}=\frac{BD}{BC} \Longrightarrow BC^2=BD\cdot AB.$$
- 由前两组相似可得 $\triangle ACD\sim\triangle CBD$，所以
$$\frac{CD}{BD}=\frac{AD}{CD} \Longrightarrow CD^2=AD\cdot BD.$$
...
### 2.5 旋转型相似（手拉手模型）【培优拓展】

![旋转型相似（手拉手模型）](../img/similarity-hand-in-hand.svg)

从点 $A$ 引出四条非重合射线 $AB,AC,AD,AE$。若
...
因此可以安全使用对应角相等和对应边成比例。至于 $BD,CE$ 交点处的角或四点共圆结论，还取决于四条射线的具体顺序和旋转方向，必须针对题图重新追角，不能作为手拉手模型的无条件结论。
...
$$\angle BAC=90^\circ,\qquad \frac{AD}{AE}=\frac{AB}{AC},\qquad \angle DAB=\angle EAC,$$

并要求从射线 $AD$ 到 $AE$、从射线 $AB$ 到 $AC$ 的旋转方向相同，相关三角形均非退化。此时由 SAS 得

$$\triangle ADB\sim\triangle AEC,\qquad BD\perp CE.$$

若 $B,C,D,E$ 按此顺序组成简单四边形，连接 $BE,CD$，还可得到

$$BE^2+CD^2=BC^2+DE^2.$$

若正交对角线 $BD,CE$ 相交于该简单四边形内部，则面积可写为

$$S_{BCDE}=\frac12 BD\cdot CE.$$

**证明要点：**

1. 由 $\dfrac{AD}{AE}=\dfrac{AB}{AC}$ 且 $\angle DAB=\angle EAC$，SAS 给出
...
![三垂直与一线三等角模型](../img/similarity-three-equal-angle.svg)
...
$$\angle 1+\angle 2=90^\circ,\qquad \angle 3+\angle 2=90^\circ$$
...
![正方形中的半角相似模型](../img/similarity-half-angle-square.svg)

图示采用如下点位：$E\in BC$、$F\in CD$，$\angle EAF=45^\circ$；正方形对角线 $BD$ 分别与 $AE,AF$ 交于 $M,N$。因此

$$\angle MAN=\angle EAF=45^\circ,\qquad \angle ABD=\angle ADB=45^\circ.$$

这些等角只是寻找相似的起点，并不能脱离点位直接套用固定比例。处理具体题目时应：

1. 明确 $M=AE\cap BD$、$N=AF\cap BD$ 以及 $E,F$ 所在边；
2. 用正方形的平行、垂直和对角线 $45^\circ$ 性质逐项计算角；
3. 写出两组对应角后再用 AA 判定相似；
4. 若需要线段和，可将一个小三角形旋转 $90^\circ$ 后再证明全等或使用勾股定理。

不同的 $E,F$ 位置或不同的辅助线会产生不同的相似三角形，图中彩色三角形只表示一种解题构造，不代表无条件通用公式。
...
$$\triangle ABE\sim\triangle DAE,\qquad \triangle DAE\sim\triangle DCA.$$
...
$$\angle BAD=\angle ACE,\qquad \angle ABD=\angle CAE.$$
...
![对角互补模型及辅助线](../img/similarity-supplementary-angle.svg)
...
在 $\mathrm{Rt}\triangle ABC$ 中，$\angle C=90^\circ$，点 $O$ 为斜边 $AB$ 的中点。取 $E\in AC$、$F\in BC$，且 $OE\perp OF$；过 $O$ 分别向 $AC,BC$ 作垂线，垂足为 $D,H$。则
...
又因 $OD\parallel BC$、$OH\parallel AC$，由三角形中位线定理可知 $D,H$ 分别是 $AC,BC$ 的中点，所以

$$OD=\frac{BC}{2},\qquad OH=\frac{AC}{2},\qquad \frac{OD}{OH}=\frac{BC}{AC},$$

故结论成立。

### 4.3 一般角中的对角互补

一般角图形中，只有两个直角和 $\angle BOC=\alpha$ 并不能唯一确定 $\dfrac{CE}{CD}$。必须先说明 $D,E$ 分别位于哪条射线或直线上，以及 $F,G$ 是哪些垂足，再由具体的两组直角三角形相似求比例。若最终出现正切，还要明确在某个直角三角形中哪一边是 $\alpha$ 的对边、哪一边是邻边。图示仅用于展示“作垂线—找余角—证相似”的方法，不提供无条件的固定比值。
...
则可确定 $A,B,C,D$ 四点共圆。过点 $D$ 分别向 $BA,BC$ 所在直线作垂线时，垂足可能在线段上、端点处或延长线上；端点重合还可能使所构造的三角形退化。因此不能只凭“对角互补”就无条件写出某一对直角三角形相似。

安全的处理顺序是：

1. 先由凸四边形一组对角互补判定四点共圆；
2. 明确两个垂足的位置，并排除三点共线等退化情形；
3. 根据同弧所对圆周角以及垂直关系，逐项证明第二组锐角相等；
4. 满足两组角对应相等后，才用 AA 判定具体三角形相似。

若图形为凹四边形或自交点序，应改用有向角重新判断，不能直接套用。

---

## 5. 十字架模型【培优拓展】

![矩形及等边三角形中的十字架模型](../img/similarity-cross-model.svg)

### 5.1 矩形中的十字架模型

在矩形 $ABCD$ 中，先看图中的基本情形：$E\in AB$，线段 $DE$ 与对角线 $AC$ 垂直，即

$$DE\perp AC.$$

此时

$$\frac{DE}{AC}=\frac{BC}{AB}.$$

更一般地，若 $E\in AB$、$F\in CD$、$M\in AD$、$N\in BC$，两条连接对边的线段满足
...
在等边三角形 $ABC$ 中，取 $D\in BC$、$E\in AC$。若

$$BD=CE$$

（或交换对应位置后满足 $CD=AE$），则可针对图中点序证明
...
![等腰直角三角形中的十字模型](../img/similarity-cross-isosceles-right.svg)

在 $\triangle ABC$ 中，若

$$AB=BC,\qquad AB\perp BC,$$

点 $D$ 为 $BC$ 中点，过 $B$ 作 $AD$ 的垂线并与 $AC$ 交于 $F$，即 $F\in AC$ 且 $BF\perp AD$，则全等与相似结合可推出一系列结论，例如
...
![一般直角三角形中的十字模型](../img/similarity-cross-general-right.svg)

若

$$BC=kAB,\qquad AB\perp BC,$$

点 $D$ 为 $BC$ 中点，过 $B$ 作 $AD$ 的垂线并与 $AC$ 交于 $F$，即 $F\in AC$ 且 $BF\perp AD$，则
...
![三角形内接矩形模型](../img/similarity-inscribed-rectangle.svg)

在 $\triangle ABC$ 中内接矩形 $EFGH$，其中 $E,F$ 在 $BC$ 上，$H,G$ 分别在 $AB,AC$ 上。由于
...
$$\angle AGH=\angle ABC,\qquad \angle AHG=\angle ACB,$$
...
## 7. 复合 A 字型与 8 字型【培优拓展】

![双 A 字型、双 8 字型](../img/similarity-combined-a8.svg)

### 7.1 双 A 字模型

同一图形中有两组平行线时，可连续使用两次 A 字型。例如

$$\triangle AEF\sim\triangle ABC,\qquad \triangle AEG\sim\triangle ABD.$$

把两组比例相除，可以消去公共边，得到新的线段比。

**证明：** 每一组平行线分别给出两组同位角相等，所以由 AA 得到两组相似。分别写出

$$\frac{EG}{BD}=\frac{AG}{AD},\qquad \frac{FG}{CD}=\frac{AG}{AD},$$
...
$$\triangle AOD\sim\triangle BOC \Longleftrightarrow \triangle AOB\sim\triangle DOC.$$
...
$$\frac{AF}{BC}=\frac{x}{x+y},\qquad \frac{AF}{DE}=\frac{y}{x+y}.$$
...
![四 A 一 8 模型](../img/similarity-a8-four-one.svg)
...
$$\frac{AB}{BC}=\frac1m,\qquad \frac{DE}{CD}=\frac1n,$$
...
可设与 $A,C,E$ 对应的权数分别为 $m,1,n$。权数法属于竞赛拓展；点 $D$ 的权数为 $n+1$，因此在 $AD$ 上
...
飞鱼模型本质上也是平行线与相似三角形的组合。例如在 $\triangle ABC$ 中，$AD$ 为中线，点 $E$ 在 $AD$ 上，直线 $CE$ 交 $AB$ 于 $F$。过 $D$ 作 $DM\parallel CF$，交 $AB$ 于 $M$，即
...
![飞鱼模型例题图及辅助线](../img/similarity-example-flying-fish.svg)
...
![对角互补例题图](../img/similarity-example-supplementary.svg)
...
![矩形十字架例题图](../img/similarity-example-rectangle-cross.svg)

在矩形 $ABCD$ 中，$E\in AD$、$F\in BC$、$G\in AB$、$H\in CD$，且
...
$$\frac{AE}{AB}=\frac24=\frac12, \qquad \frac{AD}{AC}=\frac36=\frac12.$$
...
$$\angle ADF=\angle ACG,\qquad \angle DAF=\angle CAG,$$
...
### 例 5：矩形中的 $45^\circ$ 夹角模型

![矩形中的 45 度夹角模型例题图](../img/similarity-example-half-angle.svg)
...
$$BE=\sqrt{AE^2-AB^2} =\sqrt{45-36}=3.$$
...
$$\angle AGE=\angle AHF=135^\circ,\qquad \angle GAE=\angle AFH.$$
...
$$BD=\sqrt{BF^2+DF^2} =\sqrt{36+4}=2\sqrt{10}.$$
...
$$\frac26=\frac{MG}{\sqrt{10}}, \qquad MG=\frac{\sqrt{10}}3.$$
...
- 中华人民共和国教育部：《义务教育数学课程标准（2022 年版）》，北京师范大学出版社，2022。
- 本地资料：`相似模型（教师版）.pdf`，用于核对模型结构与图形；正文证明、例题表述和配图均已重新整理。
- [教育部：义务教育课程方案和课程标准（2022 年版）](https://www.moe.gov.cn/srcsite/A26/s8001/202204/t20220420_619921.html)
- [国家中小学智慧教育平台](https://basic.smartedu.cn/)：可检索“苏科版 九年级数学 相似三角形”。
- [GeoGebra 几何工具](https://www.geogebra.org/geometry)：可动态验证对应角、对应边比例与位似关系。
- [哔哩哔哩检索：苏科版 相似三角形](https://search.bilibili.com/all?keyword=%E8%8B%8F%E7%A7%91%E7%89%88%20%E7%9B%B8%E4%BC%BC%E4%B8%89%E8%A7%92%E5%BD%A2)
