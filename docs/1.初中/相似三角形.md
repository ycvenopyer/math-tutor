...
![一线三等角（左）与三垂直（右）](../img/similarity-three-equal-angle.svg)
...
不同的 $E,F$ 位置或不同的辅助线会产生不同的相似三角形，图中只表示一种解题构造，不代表无条件通用公式。
...
飞鱼模型本质上也是平行线与相似三角形的组合。典型点位如下：

- $AD$ 是 $\triangle ABC$ 的中线（$D$ 为 $BC$ 中点）；
- $E$ 在 $AD$ 上，$CE$ 交 $AB$ 于 $F$；
- 过 $D$ 作 $DM\parallel CF$，交 $AB$ 于 $M$。

即

$$DM\parallel CF,$$

由此得到 $\triangle AFE\sim\triangle AMD$，并利用中点条件把比例转移到 $AB$ 上。

### 例

![飞鱼模型：$AD$ 中线，$CE$ 交 $AB$ 于 $F$，$DM\parallel CF$](../img/similarity-example-flying-fish.svg)
